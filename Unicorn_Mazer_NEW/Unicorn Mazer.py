...
start_size = (800, 600) # start size of the screen when the game opens
...
while True: # game main loop
    ...
    while menu:
        ...
        if event.type == QUIT: # when the player close the game
            ...
            # keep the window proporsions
            new_size = event.dict['size']
            new_size = list(new_size)
            new_size[1] = int(new_size[0]-(new_size[0]*0.2)) 

            # apply the new size into the window
            screen = pygame.display.set_mode(new_size,RESIZABLE)
            second_screen.blit(uni_right, (10, 10))
            screen.blit(pygame.transform.scale(second_screen, event.dict['size']), (0, 0))
            pygame.display.flip()